...
namespace Tourism.Controllers
{
    public class TourPackagesController : Controller
    {
        ...
        public async Task<IActionResult> ChangeCartQuantity([FromBody] ChangeQuantityDto dto)
        {
            ...
            try
            {
                ...
                if (diff > 0) // adding seats
                {
                    ...
                    {
                        // Logic handles server side validation silently/gracefully via JS if needed
        ...
        {
            var trips = await _tripRepository.GetAllTripsAsync() ?? new List<Trip>();
            return View(trips);
        }
        ...
        {
            var userId = GetCurrentUserIdString();
            if (string.IsNullOrEmpty(userId)) { TempData["Error"] = "Unable to identify user."; return RedirectToAction("Index"); }

            var cartItems = await _repo.GetCartItemsWithTripAsync(userId);
            if (cartItems == null || !cartItems.Any()) { TempData["Error"] = "Your cart is empty."; return RedirectToAction("Index"); }
        ...
        public async Task<IActionResult> Payment(CheckoutViewModel model)
        {
            ...
            const int platformOwnerId = 100;
            bool cardAllowed = false;
            bool isPlatformCard = false;

            if (creditCard.UserId.HasValue)
            {
                if (creditCard.UserId.Value == platformOwnerId) { cardAllowed = true; isPlatformCard = true; }
                else if (currentUserId.HasValue && creditCard.UserId.Value == currentUserId.Value) { cardAllowed = true; }
            }

            if (!cardAllowed)
            {
                ModelState.AddModelError("", "This card does not belong to the current user or is not permitted.");
                return View(model);
            }

            if (creditCard.Balance < model.TotalAmount)
            {
                ModelState.AddModelError("", isPlatformCard ? "Platform card has insufficient balance." : "Insufficient balance.");
                return View(model);
            }
            ...
            try
            {
                // NOTE: We do NOT check "RemainingSeats < Quantity" here because 
                // the seats were ALREADY reserved when items were added to the Cart (ChangeCartQuantity).
                ...
                foreach (var cartItem in dbCartItems)
                {
                    ...
                    if (trip != null)
                    {
                        ...
                        // NOTE: Do NOT deduct seats again here. 
                        // trip.RemainingSeats -= cartItem.Quantity; <-- REMOVED
                    }
                }
